...
class K_Means:
    ...
    def Fine_Tune(
        self, kGroups: int, threshold: float, maxEpochs: int
    ) -> tuple[list[Cluster], list[list[int | float]], int]:
        """
    ...
    def Predict(
        self, vector: list[int | float], retrain: bool = False
    ) -> tuple[list[int | float], Cluster | None]:
        """
        ...
        closestDistance: float = (
            Calc_Max_Possible_Distance(self.trainingMatrix) * self.threshold
        )
        closestCluster: Cluster = None

        for clusterObject in self.clusters:
            currentDistance: float = Calc_Distance(
                aVector=vector, bVector=clusterObject.centroid
            )
    ...
    def _Train(
        self, matrix: list[list[int | float]], k: int, threshold: float, maxEpochs: int
    ) -> tuple[list[Cluster], list[list[int | float]], int]:
        """
        ...
        while not isConverged:
            ...
            for vector in matrix:
                closestCluster: Cluster = clusters[
                    -1
                ]  # Initialize to the unassgined cluster
